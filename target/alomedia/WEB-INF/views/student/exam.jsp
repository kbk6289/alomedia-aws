		...
		<div id="content-wrapper" class="d-flex flex-column">
			...
			<div class="container-fluid">
				...
				<div
					...
					<h1 class="h2 mb-0 text-gray-800">평가 응시</h1>
				</div>
				<hr>
				<div class="input-form col-md-12">
					<form action="${context}/student/exam/${examId}" method="post">
						<c:forEach var="exam" items="${exams}" varStatus="status">
							<div>
								<label>${exam.questionNum}번 문제</label> <span style="right: 0px">점수 : ${exam.examAllotment}</span>
								...
								<input type="hidden" value="${exam.questionNum}" name="examSubmits[${status.index}].questionNum"> 
								<input type="hidden" value="${exam.examContent}" name="examSubmits[${status.index}].examContent"> 
								<input type="hidden" value="${exam.examAllotment}" name="examSubmits[${status.index}].examAllotment"> 
								<textarea class="form-control resizable-textarea" rows="3"
								name="examSubmits[${status.index}].ans" required></textarea>
							</div>
